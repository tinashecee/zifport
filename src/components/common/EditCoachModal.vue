<template>
  <div class="modal" v-if="isOpen" @click.self="close">
    <div class="modal-content edit-coach-modal">
      <span class="close-modal" @click="close" aria-label="Close">&times;</span>
      <div class="modal-header">
        <h2>Edit Coach</h2>
        <p>{{ coach ? `${coach.first_name || ''} ${coach.last_name || ''}`.trim() || 'Update coach details' : '' }}</p>
      </div>

      <form class="modal-form" @submit.prevent="handleSubmit" v-if="coach">
        <div class="form-row">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="email" class="form-input" v-model="form.email" placeholder="coach@example.com">
          </div>
          <div class="form-group">
...
import { coachesAPI, clubsAPI } from '@/services/api'
...
function fillForm(c) {
  ...
  form.license_level = c.license_level ?? 'Local'
  form.club_id = c.club_id ?? ''
  form.years_of_experience = c.years_of_experience ?? 0
  form.status = c.status ?? 'active'
  form.availability = c.availability ?? 'available'
  form.email = c.email ?? ''
  form.phone = c.phone ?? ''
  form.address = c.address ?? ''
  form.photo_url = c.photo_url ?? ''
}

watch(() => [props.isOpen, props.coach], () => {
  if (props.isOpen && props.coach) {
    fillForm(props.coach)
    error.value = ''
...
async function handleSubmit() {
  ...
  try {
    ...
    const data = await coachesAPI.update(props.coach.id, payload)
    const updated = data.coach ?? data
    emit('saved', updated)
...
.modal-content.edit-coach-modal {
  background: white;
  padding: 32px;
